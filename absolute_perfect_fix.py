...
# This is the absolute perfect code for train_unet.py for Massachusetts dataset
perfect_train_unet = """import sys
import os
...
for i, cell in enumerate(nb["cells"]):
    if cell["cell_type"] == "code" and "%%writefile models/train_unet.py" in "".join(cell.get("source", [])):
        # Construct exact source list properly
        lines = perfect_train_unet.split("\\n") # split by literal \n? No, actual \n!
        lines = perfect_train_unet.split("\n")
        source = ["%%writefile models/train_unet.py\n"]
        for line in lines:
            source.append(line + "\n")
        
        # Remove the very last newline from the last element to avoid \n at EOF, although \n is fine.
...
print("Absolute perfect cell fixed!")
